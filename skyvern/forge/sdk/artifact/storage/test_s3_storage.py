...
def _assert_object_meta(boto3_test_client: S3Client, uri: str) -> None:
    ...
    boto3_test_client.get_object_tagging(Bucket=TEST_BUCKET, Key=s3uri.key)
    tags = boto3_test_client.get_object_tagging(Bucket=TEST_BUCKET, Key=s3uri.key)
    assert tags["TagSet"] == [
        {"Key": "dummy", "Value": f"org-{TEST_ORGANIZATION_ID}"},
        {"Key": "test", "Value": "jerry"},
    ]
